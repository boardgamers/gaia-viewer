<template>
  <g :class='["scoringTile", {highlighted, faded}]' v-b-tooltip :title="tooltip">
    ...
    <text class="content" x="5" y="31" v-if="!['ts','m', 'step', 'mg', 'a'].includes(event.condition)">{{content.split(" ")[0]}}</text>
    <Condition :condition=event.condition :transform="`translate(${(event.condition === 'step' || event.condition === 'a') ? 27 : 34}, ${event.condition === 'step' ? 20 : 22}) scale(1.3)`" />
