  ...
  <div class="player-info no-gutters" v-if="player && player.faction">
    ...
    <div class="board mt-2">
      <svg viewBox="-0.2 -0.5 38.5 21.4" class="player-board" :style="`background-color: ${factionColor}`">
        ...
        <SpecialAction v-for="(action, i) in player.actions" :action="player.events['=>'].map(x => x.toString())" :disabled="!action.enabled || passed" :key="action.action + '-' + i" y=17.5 width=3.1 height=3.1 :x=3.3*i />
      </svg>
